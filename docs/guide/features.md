...
> 以下特性基于 **v3.1.5** 整理，与上游仓库 [xiaoyaya191/bilibili_learning_bot](https://github.com/xiaoyaya191/bilibili_learning_bot) 保持一致。
...
| ❤️ 好感度系统 | 按用户维度追踪互动好感值，AI 自主加减分，面板可视化管理（默认关闭） |
| 🧠 AI 技能提炼 | 视频归档时自动提炼可复用方法论为技能卡片，Agent 私信时检索注入 |
| 💬 AI 分段回复 | 私信先回"让我看看"，看完视频后再回来总结回复，拟人节奏 |
| 🧮 模型可用性测试 | 多线程逐个测试模型（自定义并发/超时/提示词），一键区分可用/下线/超时 |
| 🎯 学习小目标 | AI 自动设定学习目标 + 手动增删改查 + 进度追踪 |
| 📧 邮箱找回密码 | 首次使用引导配置邮箱，忘记密码邮件验证码找回，支持备用邮箱 |
| ⚙️ 端口/托盘自定义 | 自定义面板端口，可选隐藏系统托盘图标 |
| 🆕 新手教程 | 首次进入自动展开在系统仪表盘，7 步带真实完成状态检测 |
